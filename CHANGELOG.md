...
- **Auto-Updater System** - Added an in-game auto-updater (`urw_updater.rpy`) to check for, download, and install the latest updates directly from GitHub.
- **Dynamic Text Resizing** - Added a slider to dynamically scale the in-game choice text size, along with an auto-adapt toggle that smoothly adjusts choice button heights to prevent text overlap.
- **Custom Choice Menu Toggle** - Added a setting to easily switch between the game's default choice menu and URW's specialized dark animated choice menu.

### 🐛 Bug Fixes & Stability
- **Settings Persistence Fix** - Completely removed legacy URW 1.x migration code that was causing the `urw_enabled` setting to forcefully reset to false upon game reload.
- **AST Parsing Fixes** - Fixed an issue where the AST parser would crash on certain unusual object types, and resolved an indentation mismatch error in `urw_screens.rpy`.
